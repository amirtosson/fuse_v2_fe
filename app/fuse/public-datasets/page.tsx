...
async function getDatasets(searchParams: any) {
    ...
    // Construct Fields Object based on filter params
    const fieldsObj: any = {};
    if (searchParams.instrument) fieldsObj["scientificMetadata.Experiment.instrument"] = searchParams.instrument;
    if (searchParams.probe) fieldsObj["scientificMetadata.Experiment.probe"] = searchParams.probe;
    if (searchParams.category) fieldsObj["scientificMetadata.Sample.category"] = searchParams.category;
    if (searchParams.type) fieldsObj["type"] = searchParams.type;
    if (searchParams.pi) fieldsObj["principalInvestigator"] = searchParams.pi;
    if (searchParams.location) fieldsObj["creationLocation"] = searchParams.location;

    // Build final URL
    const url = new URL('https://public-data.desy.de/api/v3/datasets/fullquery');
    url.searchParams.append('limits', JSON.stringify(limitsObj));
    if (Object.keys(fieldsObj).length > 0) {
        url.searchParams.append('fields', JSON.stringify(fieldsObj));
    } else {
            ...
            <header className="border-b border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md sticky top-0 z-50">
                <div className="max-w-[90rem] mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white tracking-widest uppercase"><span className="text-cyan-500">DESY</span> / PUBLIC DATA</h1>
                    </div>
                </div>
            </header>
            ...
            <main className="flex-1 max-w-[90rem] mx-auto px-6 py-12 w-full flex flex-col md:flex-row gap-8">
                ...
                <div className="flex-1 w-full min-w-0">
                    <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-gray-200 dark:border-gray-800">
                        <div>
                            <h2 className="text-4xl font-black text-gray-900 dark:text-white mb-2 tracking-tighter uppercase">Data Registry Hub</h2>
                            <p className="text-gray-500 dark:text-gray-400 max-w-2xl text-lg font-medium tracking-tight">Live scientific endpoints routed via `datasets/fullquery`.</p>
                        </div>
                        {/* View Toggle */}
                    ...
                    {error ? (
                        ...
                        <>
                                    ...
                                    <Link href={`/fuse/public-datasets/${encodeURIComponent(ds.pid)}`} key={ds.pid} className={`group flex bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 hover:border-cyan-500/50 rounded-[2rem] transition-all shadow-xl hover:shadow-cyan-500/10 active:scale-[0.98] ${isList ? 'flex-col sm:flex-row items-start sm:items-center gap-6 p-4 px-6' : 'flex-col p-8'}`}>
                                        ...
                                        <div className={isList ? "flex-1 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8 w-full min-w-0" : "flex flex-col h-full w-full"}>
                                            <div className={isList ? "flex-1 min-w-0" : ""}>
                                                <div className={`text-[10px] font-black text-cyan-600 dark:text-cyan-500 tracking-widest uppercase flex items-center gap-2 ${isList ? 'mb-2' : 'mb-4'}`}>
                                                ...
                                                {!isList && (
                                                    <div className="text-xs text-gray-500 dark:text-gray-400 space-y-3 mb-8 flex-1 bg-gray-50 dark:bg-gray-950 p-6 rounded-2xl border border-gray-100 dark:border-gray-800">
                                                        <div className="flex justify-between items-center"><span className="text-gray-400 dark:text-gray-500 font-black uppercase tracking-widest text-[9px]">Instrument</span> <span className="text-emerald-600 dark:text-emerald-400 font-mono font-bold">{ds.scientificMetadata?.Experiment?.instrument || '-'}</span></div>
                                                        <div className="flex justify-between items-center"><span className="text-gray-400 dark:text-gray-500 font-black uppercase tracking-widest text-[9px]">Probe</span> <span className="text-gray-900 dark:text-white font-mono font-bold italic">{ds.scientificMetadata?.Experiment?.probe || '-'}</span></div>
                                                        <div className="flex justify-between items-center truncate"><span className="text-gray-400 dark:text-gray-500 font-black uppercase tracking-widest text-[9px] shrink-0">Sample Category</span> <span className="text-gray-900 dark:text-white truncate ml-2 font-black">{ds.scientificMetadata?.Sample?.category || '-'}</span></div>
                                                    </div>
                                                )}
                                            </div>
                            ...
                            <div className="mt-12 flex items-center justify-between p-5 bg-gray-50 dark:bg-gray-900 rounded-[2rem] border border-gray-200 dark:border-gray-800 shadow-xl">
                                ...
                                <span className="text-gray-400 dark:text-gray-500 font-black uppercase tracking-widest text-[10px] hidden sm:block">Context Node <span className="text-cyan-600 dark:text-cyan-400 font-black text-xl ml-2">{currentPage}</span></span>
                                <Link
