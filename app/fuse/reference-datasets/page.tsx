    ...
    return (
        <div className="min-h-screen bg-background text-foreground font-sans flex flex-col transition-colors duration-300">
            <header className="border-b border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 sticky top-0 z-50 backdrop-blur-md">
                <div className="max-w-[90rem] mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white tracking-widest"><span className="text-emerald-500">FUSE</span> / REFERENCE DATASETS</h1>
                    </div>
                </div>
            </header>
            ...
            <main className="flex-1 max-w-[90rem] mx-auto px-6 py-12 w-full">
                <div className="mb-12">
                    <h2 className="text-4xl font-black text-gray-900 dark:text-white mb-4">Benchmark Library</h2>
                    <p className="text-gray-500 dark:text-gray-400 text-lg max-w-3xl">Curated calibration sets and "Gold Standard" datasets used for instrument verification and comparative scientific analysis within the FUSE platform.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {referenceDatasets.map((ds) => (
                        <div key={ds.id} className="group relative bg-white dark:bg-gray-900/40 border border-gray-200 dark:border-gray-800 hover:border-emerald-500/50 rounded-3xl p-8 transition-all shadow-2xl hover:shadow-emerald-900/20 active:scale-[0.99] overflow-hidden">
                            {/* Accent Decoration */}
                            ...
                            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-3 group-hover:text-emerald-600 dark:group-hover:text-emerald-300 transition-colors">{ds.name}</h3>
                            <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed mb-8 line-clamp-3">{ds.description}</p>
                            
                            <div className="grid grid-cols-2 gap-4 mb-8">
                                <div className="bg-gray-50 dark:bg-gray-950 p-3 rounded-xl border border-gray-200 dark:border-gray-800">
                                    <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1 opacity-70">Instrument</p>
                                    <p className="text-sm text-emerald-700 dark:text-emerald-100 font-bold truncate">{ds.instrument}</p>
                                </div>
                                <div className="bg-gray-50 dark:bg-gray-950 p-3 rounded-xl border border-gray-200 dark:border-gray-800">
                                    <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1 opacity-70">Technique</p>
                                    <p className="text-sm text-cyan-700 dark:text-cyan-100 font-bold truncate">{ds.type}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between pt-6 border-t border-gray-100 dark:border-gray-800">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left">
                                        <p className="text-xs font-bold text-gray-900 dark:text-gray-200">{ds.pi}</p>
                                        <p className="text-[10px] text-gray-500 font-black uppercase tracking-tighter opacity-70">Principal Investigator</p>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                     <button className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-600 dark:text-emerald-400 hover:text-gray-950 rounded-lg text-xs font-bold transition-all border border-emerald-500/20">
                                         Preview
                                     </button>
                                     <button className="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-900 dark:text-white rounded-lg text-xs font-bold transition-all border border-gray-200 dark:border-gray-700">
                                         Download
                                     </button>
                                </div>
                            </div>
